...
def evaluate(model, dl):
    ...
    with torch.no_grad():
        for i, data in enumerate(dl):
            ...
            loss = F.binary_cross_entropy(outputs, targets.float().to(device))
...
def train(config=None):
    ...
    if True:
        ...
        for split_id, (train_index, test_index) in enumerate(kfolds):
            ...
            model = NLP2023MetaNetwork2b(raw_size= config.raw_size, feat_size=config.feat_size,
...
config.raw_size = 441 # diff of grads
config.feat_size = 30
config.hidden_size = 60
config.feature_name = "action_grads_valuediff"
# train(config)
