...
from rljuly2023_featuresv3 import *
...
def predict(saved_config):
    ...
    with torch.no_grad():
        ...
        for fold_state in kfold_state_dicts:
            ...
            model = NLP2023MetaNetwork(**saved_config)
...
class TrojFeatures(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {"feats": torch.FloatTensor(features)}, label
...
def train_final(config=None):
    print("Training...")
    if True:
        meta_df = get_metadata()
        model = NLP2023MetaNetwork(raw_size=100, feat_size=60, hidden_size=30, nlayers_1=config.nlayers_1)
        optimizer = torch.optim.Adam(model.parameters(), lr=config.lr)
        train_ds = TrojFeatures(meta_df)
        ...
        for epoch in range(config.epochs):
            ...
            print(f'Epoch {epoch:3d} train_loss {avgtrain_loss:.2f} AUC: {train_auc:.2f}')
...
def train(config=None):
    ...
    if True:
        ...
        for split_id, (train_index, test_index) in enumerate(kfolds):
            ...
            model = NLP2023MetaNetwork(raw_size=100, feat_size=30, hidden_size=30, nlayers_1=config.nlayers_1)
            optimizer = torch.optim.Adam(model.parameters(), lr=config.lr)
            # scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, config.epochs, eta_min = config.lr / 100)
